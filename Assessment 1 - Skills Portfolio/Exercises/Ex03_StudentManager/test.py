...
def loadStudents(filePath=filePath):
    students = []
    with open(filePath, "r") as file:
        ...
        for _ in range(class_count):
            ...
            student_number, name, c1, c2, c3, exam_mark = line.split(",")

            c1, c2, c3, exam_mark = int(c1), int(c2), int(c3), int(exam_mark)
...
def viewIndividual():
    ...
    query = query.lower()
    for s in students:
        if query == s["student_number"] or query in s["name"].lower():
...
# TEXT BOX
text_box = tk.Text(main, width=70, height=20, font=("Arial", 14))
